...
if __name__ == "__main__":
    ...
    output_file = "/home/ubuntu/logbert_project/output/anomaly_results.json"
    ...
    print("\nTop cloud alerts saved as JSON:")
    ...
    with open(output_file, "a", encoding="utf-8") as f:
        for row in results[:10]:
            json_row = {
                ...
                "reason": row["reason"],
                "sample_logs": row["sample_logs"],
                "recommendation_required": row["anomaly_score"] >= 10
            }

            print(json.dumps(json_row, indent=2))
            f.write(json.dumps(json_row) + "\n")

    print(f"\nJSON anomaly results saved to: {output_file}")