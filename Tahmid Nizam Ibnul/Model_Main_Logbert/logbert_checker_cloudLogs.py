...
# 1. Local paths
# =========================
BASE_DIR = "/home/ubuntu/logbert_project/model"
...
if __name__ == "__main__":
    ...
    output_file = "/home/ubuntu/logbert_project/output/anomaly_results.log"
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("Top cloud alerts:\n\n")

        for row in results[:10]:
            line_block = []
            line_block.append("=" * 80)
            line_block.append(f"Window ID   : {row['window_id']}")
            line_block.append(f"Lines       : {row['start_line']}-{row['end_line']}")
            line_block.append(f"Score       : {row['anomaly_score']:.4f}")
            line_block.append(f"Alert Level : {row['alert_level']}")
            line_block.append(f"Alert Type  : {row['alert_type']}")
            line_block.append(f"Reason      : {row['reason']}")
            line_block.append("Sample logs:")
            for log_line in row["sample_logs"]:
                line_block.append(f" - {log_line}")
            line_block.append("")
